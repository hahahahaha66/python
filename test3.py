...
def generate_patterns_with_defects(material_len, defects):
    ...
    for counts in itertools.product(range(0, 6), repeat=8):
        if sum(counts) == 0:
            continue
        segment_positions = []
        pos = 0.0
        ...
        for i, count in enumerate(counts):
            ...
            for _ in range(count):
                total_len = edge_len + saw_kerf if len(segment_positions) > 0 else edge_len
                # 跳过缺陷区域
                while overlaps_with_defects(pos, total_len, defects):
                    pos += 0.001  # 小步跳过（可调节精度）
